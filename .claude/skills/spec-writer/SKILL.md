---
name: spec-writer
description: Generates technical implementation spec and plan for one or more features based on PRD, codebase analysis, and iterative clarification. The contract is a stack-agnostic, consumer-agnostic GTW-style file (`contract.md`) describing the testable promise of the feature. Supports batch mode for generating multiple features from the same wave in parallel.
---
...
3. `contract.md` - Behavior contract (verification surfaces + GTW items + coverage manifest); the testable promise of the feature, written in consumer-agnostic language so any tool, agent, or human can read and exercise it.
...
`contract.md` is a **sibling** of `spec.md`, not as child: both are generated in the same run from the same PRD, but the contract´s content is grounded in PRD section 9 acceptance criteria (not in spec´s implementation choices), so refatoring the implementation never affects the contract.
...
**optional flag:** the input my include `create-issue` (anywhere) when present, the skill create a GitHub traccking issue for the feature in step 6 will ask the user `y/n` before creating: when ansent in batch mode, the orchestrator´s consolidate plan(B.4) asks once and the awswer applies to every feature in the batch. See **Step 6** for issue creation flow and **bath mode** for the batch interaction.
...
When the input contains multiple feature IDs, a wave reference (`wave N`), or a mix, the skill enters Batch Mode. Full orchestration protocol, Auto-Accept Policy, and batch-specific rules are in `references/batch-mode.md`.

**Batch Mode activation signals:** multiple IDs (`F01 F02`), wave reference (`wave 3`), or a mix. Single-feature input stays on the interactive flow.
...
**Always:**
- Generate TWO files (spec and plan) + `contract.md` in `docs/<feature-id>-<kebab-name>/`
- Validate both documents before saving
- Run Codebase Pattern Discovery in two layers (baseline + broad) before the interview
- Use PRD Consumes/Provides/Core Scope/Full Scope/Capabilities/Experience/Error Handling/acceptance criteria as primary context
- Skip interview questions whose answers are already in the PRD, codebase, or previous specs
...
- Include time estimates, Feature ID/Date/Version metadata, or implementation details in plan steps
- Create testing phases in plan document
- Proceed without a PRD — direct the user to `prd-writer` if absent
- Re-ask questions answerable from codebase or PRD
- Restrict codebase exploration to the baseline checklist — it is a floor, not a ceiling

---

## Edge Cases

**No PRD found:** Stop; instruct user to run `prd-writer` first.
**Feature not found in PRD:** List available features from Section 8.
**Ambiguous feature reference:** List candidates; ask user to disambiguate.
**Multiple PRD files:** Ask user which to use.
**Dependency not yet implemented:** Warn and proceed only if confirmed.
**Empty/scaffolded-only codebase:** Skip Pattern Discovery; ask transversal stack questions in Step 2.
**PRD has no Core Scope / Full Scope blocks:** Assume full feature scope.
**Description too vague:** Go deeper in the interview; do not assume defaults silently.
**No codebase patterns found (non-empty codebase):** Ask user to confirm best-practice defaults.
**New technologies not in codebase:** List, ask for confirmation, document in decisions.
**Multiple conflicting patterns:** Present both; ask which to follow; document choice.
**Kebab-case sanitizing:** lowercase, spaces → hyphens, strip `[^a-z0-9-]`. Example: `F07. Background Video Processing Pipeline` → `F07-background-video-processing-pipeline`.
**Batch-specific edge cases:** see `references/batch-mode.md`.
